                            ...
                            {model_data.data?.data.map((row, index) => (
                                <div
                                    ...
                                    className={` bg-linear-to-br from-gray-50 to-gray-100 group 
                                             ...
                                             ${selectedElements.has(`${row.Element_Type}-${row.Level}`) ? `shadow-lg shadow-blue-500/25 text-[${COLORS_SET[index % COLORS_SET.length]}]` : ''}`}
                                style={{
                                    background: selectedElements.has(`${row.Element_Type}-${row.Level}`) ? hexToRgba(COLORS_SET[index % COLORS_SET.length], 0.6) : '',
                                }}
                                >
